...
import { ProfessionalUserResponse, ProjectValues } from "@/types/user";
...
const servicesData = [
  ...
  },
  {
    ...
    price: 15000,
    description: "Instalação de toda rede elétrica para residências até 150m²",
  },
];
...
const ProfessionalModal = ({
  ...
  const [selectedService, setSelectedService] = useState<ProjectValues | null>(
    null,
  );
  const [isImageModalOpen, setImageModalOpen] = useState(false);
  const [currentImageIndex, setCurrentImageIndex] = useState(0);

  const { data } = useQuery<ProfessionalUserResponse>({
  ...
  const openImageModal = (project: ProjectValues) => {
              ...
              <div className="md:col-span-2">
                <Tabs defaultValue="about">
                  ...
                  <TabsContent value="services" className="mt-4">
                    ...
                    <div className="flex flex-col gap-4">
                      {servicesData.map(({ name, description, price }) => (
                        <div
                          key={name}
                          className="py-2 px-4 border border-gray-200 rounded-md shadow-sm bg-white"
                        >
